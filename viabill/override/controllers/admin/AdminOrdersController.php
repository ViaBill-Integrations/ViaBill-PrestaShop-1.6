...
class AdminOrdersController extends AdminOrdersControllerCore
{
    ...
    public function postProcess()
    {
         ...
         */
        $orderHandler = $module->getContainer()->get('service.handler.paymentManagement');
    ...
    public function displayCapturePaymentLink($token, $idOrder)
    {
        ...
        $orderStatus = $module->getContainer()->get('service.provider.orderStatus');
        if (!$orderStatus->canBeCaptured($order)) {
            return;
        }

        /** @var \ViaBill\Builder\Template\ListButtonTemplate $listButton */
        $listButton = $module->getContainer()->get('builder.template.listButton');
        /** @var \ViaBill\Install\Tab $tab */
        $tab = $module->getContainer()->get('tab');
        ...
        $link = $module->getContainer()->get('adapter.link');

        $actionsLink = $link->getAdminLink(
        ...
        if (!$module->isPS16()) {
            $listButton->setIconLink('../img/admin/money.gif');
        }
    ...
    public function displayRefundPaymentLink($token, $idOrder)
    {
        ...
        if (!$module->isPS16()) {
            $listButton->setIconLink('../img/admin/arrow_down.png');
        }
    ...
    public function displayCancelPaymentLink($token, $idOrder)
    {
        ...
        if (!$module->isPS16()) {
            $listButton->setIconLink('../img/admin/icon-cancel.png');
        }
    ...
    private function isNotViaBillOrder(Order $order, ViaBill $module)
    {
         ...
         */
        $config = $module->getContainer()->get('config');
